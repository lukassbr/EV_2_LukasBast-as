...
fetch("http://144.126.210.74:8080/api/gestion", requestOptions)
  .then((response) => {
    if(response.status == 200){
      mostrarAlerta("Gestión agregada correctamente", 'success');
      setTimeout(() => {
...
.then((response) => {
  if(response.status == 200){
    mostrarAlerta("Tipo de gestión eliminado correctamente", 'success');
    setTimeout(() => {
      location.href = "listar.html";
    }, 2000);
  }
  else if(response.status == 400){
    mostrarAlerta("No es posible eliminar. Registro esta siendo utilizado.", 'danger');
  }
})
.then((result) => console.log(result))
.catch((error) => console.error(error));
